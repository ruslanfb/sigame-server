...
const statusBar = (b) => { A(b, storage.text('9:41', 24, 14, { size: 14, weight: 600, name: 'time' })); A(b, storage.text('SIGAME', W / 2 - 36, 12, { font: 'display', size: 16, weight: 600, upper: true, color: T.accent2, letterSpacing: 2, name: 'brand' })); };
...
const scoreStrip = (b, me = 1) => {
  ...
  names.forEach((n, i) => {
    ...
    A(b, storage.text(n, x + 12, y + 10, { size: 13, color: T.muted }));
    A(b, storage.text(scores[i], x + 12, y + 28, { font: 'display', size: 24, weight: 600, color: scores[i].startsWith('-') ? T.danger : T.text }));
  });
};
...
const input = (b, placeholder, y, value) => {
  ...
  A(b, storage.text(value || placeholder, 40, y + 16, { size: 16, color: value ? T.text : T.muted }));
};

// 01 Join
{ const b = mk(1, 'Join'); statusBar(b);
  A(b, storage.text('СВОЯ ИГРА', 24, 120, { font: 'display', size: 48, weight: 600, upper: true, color: T.accent2, w: W - 48, align: 'center' }));
  A(b, storage.text('Введите код комнаты', 24, 200, { size: 16, color: T.muted, w: W - 48, align: 'center' }));
  ...
  A(b, storage.text('или отсканируйте QR с экрана ведущего', 24, 550, { size: 13, color: T.muted, w: W - 48, align: 'center' }));
}
// 02 Lobby
{ const b = mk(2, 'Lobby'); statusBar(b);
  A(b, storage.text('Комната K7M3P', 24, 60, { font: 'display', size: 32, weight: 600, upper: true, color: T.accent2 }));
  A(b, storage.text('Пятничная игра · пак «Тестовый пакет»', 24, 104, { size: 14, color: T.muted, w: W - 48 }));
  ...
  people.forEach(([n, s, ok], i) => {
    ...
    A(b, storage.text(n, 64, y + 16, { size: 16, weight: 600 })); A(b, storage.text(s, W - 120, y + 18, { size: 13, color: ok ? T.success : T.muted }));
  });
  primaryButton(b, 'Я готов', 430);
  A(b, storage.text('Ждём, пока ведущий начнёт игру…', 24, 500, { size: 14, color: T.muted, w: W - 48, align: 'center' }));
  ...
  A(b, storage.text('Чат', 40, 572, { size: 12, color: T.muted, upper: true }));
  A(b, storage.text('Борис: всем привет!', 40, 596, { size: 14 })); A(b, storage.text('Вера: погнали', 40, 620, { size: 14 }));
  input(b, 'Сообщение…', 680);
}
// 03 Table (choose a question)
{ const b = mk(3, 'Table'); statusBar(b);
  A(b, storage.text('Ваш ход — выберите вопрос', 24, 56, { size: 16, weight: 600, color: T.accent2, w: W - 48 }));
  const themes = ['История', 'Кино', 'Наука', 'Спорт', 'Музыка'];
  themes.forEach((t, r) => {
    const y = 100 + r * 120;
    A(b, storage.text(t, 24, y, { size: 13, weight: 600, color: T.muted, upper: true }));
...
{ const b = mk(4, 'Question'); statusBar(b); header(b, 'КИНО', '300');
  A(b, storage.text('Этот режиссёр снял «Начало», «Интерстеллар» и «Оппенгеймер».', 24, 176, { size: 22, weight: 600, w: W - 48, h: 120, align: 'center' }));
  ...
  const t = storage.text('ЖМИ!', 24, 360, { font: 'display', size: 72, weight: 600, color: T.accentContrast, align: 'center', w: W - 48, h: 300 }); if (t) { t.verticalAlign = 'center'; b.appendChild(t); }
  A(b, storage.text('пинг 12 мс · точность ±3 мс', 24, 680, { size: 12, color: T.muted, w: W - 48, align: 'center' }));
  scoreStrip(b, 0);
}
// 05 Answer input
{ const b = mk(5, 'Answer'); statusBar(b); header(b, 'КИНО', '300');
  ...
  A(b, storage.text('Этот режиссёр снял «Начало», «Интерстеллар» и «Оппенгеймер».', 24, 240, { size: 18, w: W - 48, h: 90, color: T.muted, align: 'center' }));
  ...
  A(b, storage.text('осталось 18 с', 24, 430, { size: 12, color: T.muted, w: W - 48, align: 'center' }));
  primaryButton(b, 'Ответить', 470);
  scoreStrip(b, 0);
}
// 06 Stake dialog
{ const b = mk(6, 'Stake'); statusBar(b); header(b, 'ВОПРОС СО СТАВКОЙ', '400');
  A(b, storage.text('Ваша ставка', 24, 176, { size: 16, color: T.muted, w: W - 48, align: 'center' }));
  ...
  A(b, storage.text('мин. 500 · макс. 1 300 · шаг 100', 24, 372, { size: 12, color: T.muted, w: W - 48, align: 'center' }));
...
{ const b = mk(7, 'Verdict'); statusBar(b); header(b, 'КИНО', '300');
  ...
  const t = storage.text('ВЕРНО  +300', 24, 220, { font: 'display', size: 44, weight: 600, color: T.accentContrast, align: 'center', w: W - 48, h: 160 }); if (t) { t.verticalAlign = 'center'; b.appendChild(t); }
  A(b, storage.text('Правильный ответ: Кристофер Нолан', 24, 410, { size: 16, color: T.muted, w: W - 48, align: 'center' }));
